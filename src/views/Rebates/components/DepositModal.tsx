...
const DepositModal: React.FC<DepositModalProps> = ({ max, onConfirm, onDismiss, tokenName = '', token }) => {
  ...
  function formatOutAmount() {
    ...
    return `Receiving: ${outAmount.toFixed(4)} CASH ($${(outAmount * rebateStats.tombPrice * ftmPrice).toFixed(2)})`
  }
  ...
  return (
    <Modal>
      ...
      <StyledMaxText style = {{color: getOutAmount() < rebateStats.tombAvailable ? "black" : "var(--accent)"}}>
        {rebateStats.tombAvailable > 0 ? `${rebateStats.tombAvailable.toFixed(4)} CASH Available` : "Bond Sold Out"}
      </StyledMaxText>
      <ModalActions>
